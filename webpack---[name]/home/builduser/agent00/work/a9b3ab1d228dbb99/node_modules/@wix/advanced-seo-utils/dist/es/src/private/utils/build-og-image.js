...
export function buildOgImagePreview(url, width, height, method) {
    ...
    return "https://static.wixstatic.com/media/" + urlOrName + "/v1/" + method + "/w_" + width + ",h_" + height + ",al_c/" + urlOrName;
}
...
function getOgImageMeta(tags) {
    var ogImage = tags.find(function(tag) {
        ...
        if (type === TAG_TYPES.META) {
            return props[PROPS.PROPERTY] === 'og:image';
        }
        return false;
    });
    return (ogImage || {}).meta;
...
function extractImageNameFromWixMediaUrl(url) {
