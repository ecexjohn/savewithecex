...
function build(
	...
	if (window?.consentPolicyManager) {
		const consentPolicy = window.consentPolicyManager!.getCurrentConsentPolicy()
		shouldCleanBeats = !(consentPolicy?.policy.functional && consentPolicy?.policy.analytics)
	}
	...
	const biParams = {
		...
		sessionId: shouldCleanBeats && window?.consentPolicyManager ? OMIT_PARAM : fromSession('sessionId'),
		siterev:
		...
		_brandId: sessionData.commonConfig ? sessionData.commonConfig.brand : OMIT_PARAM,
		nt: extraParamHandler('nt', OMIT_PARAM),
	}
	const query: string = Object.entries(biParams)
		.map(getQueryParamBuilder)
		.filter(Boolean)
		.join('&')
