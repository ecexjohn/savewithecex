...
const PHONE_NUMBER_PATTERN = /(?:\+|\()?\d(?:[-.() \t\u00a0\u1680\u180e\u2000\u200a\u2028\u2029\u202f\u205f\u3000\ufeff]{0,5}\d){6,16}\)?|\*\d{4}/
...
const GLOBAL_PHONE_NUMBER_PATTERN = new RegExp(PHONE_NUMBER_PATTERN, 'g')
const GLOBAL_EMAIL_PATTERN = new RegExp(EMAIL_PATTERN, 'gi')
const GLOBAL_URL_PATTERN = new RegExp(URL_PATTERN, 'gi')
...
const findUrl: MatchingFunc = (text) => {
	...
	if (matching) {
		const mainCapture = matching[2]
		const prefixSize = matching[1].length
		const beginsWithHttp = matching[2].toLowerCase().indexOf('http') === 0
...
export const replacePhoneNumbers = (str: string, replacer: TextReplacer) =>
	str.replace(GLOBAL_PHONE_NUMBER_PATTERN, replacer)
