			...
			return {
				boundingRectAndOffsets(compId: string): HandlerResponse {
					const result = {
						...
						scale: 1, // TODO - eventually should be taken from santaTypesDefinitions.RenderFlags.siteScale
					}
					...
					result.offsets = {
						x: rect.left + window!.document.body.scrollLeft,
						y: rect.top + window!.document.body.scrollTop - headerHeight,
					}
					result.rect = _(rect)
